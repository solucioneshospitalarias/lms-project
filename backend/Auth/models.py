...
class CustomUserManager(BaseUserManager):
    ...
    def create_user(
        self, email, num_documento, user_type, password=None, **extra_fields
    ):
        if not email:
...
class CustomUser(AbstractBaseUser, PermissionsMixin):
    ...
    user_type = models.CharField(
        max_length=10, choices=USER_TYPE_CHOICES, verbose_name="Tipo de usuario"
    )
...
class PerfilProfesor(models.Model):
    ...
    estado_acc_profe = models.CharField(
        max_length=20, choices=ESTADO_CHOICES, default="activo"
    )
...
class ProfesorGrados(models.Model):
    ...
    profesor = models.ForeignKey(
        PerfilProfesor, on_delete=models.CASCADE, related_name="grados_asignados"
    )
